...
MyString::MyString() {
   ...
   data = "";  // TODO: Initialize with empty string

}

MyString::MyString(const string& str) {

   data = str;  // TODO: Initialize data with the given std::string
...
MyString::MyString(const char* str) {
   ...
   if (str != nullptr) {
       ...
       data = str;  // TODO: Initialize data with the given C-string
...
string MyString::getData() const {
   ...
   return data;  // TODO: Return the internal string data
...
int MyString::length() const {
   ...
   return static_cast<int>(data.length());  // TODO: Return the length of the string
...
MyString MyString::substring(int start, int len) const {
   ...
   // Adjust len if it exceeds string bounds

   int maxLen = static_cast<int>(data.length()) - start;

   int actualLen = (len > maxLen) ? maxLen : len;

   return MyString(data.substr(start, actualLen));
...
MyString MyString::trim() const {
   ...
   if (data.empty()) return MyString();

   size_t first = data.find_first_not_of(" \t\n\r");

   if (first == string::npos) return MyString();  // All whitespace
...
MyString MyString::replace(const string& oldStr, const string& newStr) const {
   ...
   if (oldStr.empty()) return *this;  // Edge case: nothing to replace
   ...
   while ((pos = result.find(oldStr, pos)) != string::npos) {
       ...
       pos += newStr.length();  // ⚠️ Advance past the replacement to avoid infinite loop
...
MyString MyString::operator*(int times) const {
   ...
   result.reserve(data.length() * times);  // Performance optimization
